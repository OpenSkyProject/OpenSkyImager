 ...
 */
 /*
 * This file has been patched using the proposed (and never accepted) patch
 * from Mr. Johannes Stezenbach.
 * Story of this patch and reasons that forced me to apply can be found here:
 * http://sourceforge.net/p/libusb/mailman/libusb-devel/thread/20120110155527.GA31755%40sig21.net/#msg28654569
 * 
 * On top of this a means to split gather/scatter transfer when above a certain
 * size was also add.
 * Experiments have shown that even a single urb with size above DoS limit
 * is going to fail. Thus we need to split and use the "retry" feature to
 * keep the transfer alive.
 * Due to the above and the promise to adopt "a better solution", wide use of
 * this patched version is not recommended.
 * It's provided together with OpenSkyImager as a temporary fix to overcome
 * an inherent bulk transfer size limit that is a consequence of the 
 * introduction of a "flying urbs" number limit.
 * This fix will be promptly withdrawn (with relief) as soon as libusb 
 * developers will adopt the promised "better fix".
 * Intended use is to be statically linked to OpenSkyImager. Use from within
 * other packages is of course possible, but not encouraged unless one has
...
static int submit_urbs(struct usbi_transfer *itransfer,
		       struct libusb_transfer *transfer,
		       struct linux_transfer_priv *tpriv,
		       unsigned int num_urbs)
{
	...
	for (i = tpriv->num_submitted; num_urbs-- && i < tpriv->num_urbs; i++) {
		struct usbfs_urb *urb = &tpriv->urbs[i];

		r = ioctl(dpriv->fd, IOCTL_USBFS_SUBMITURB, urb);
		if (r >= 0) {
			usbi_dbg("submitted URB %d", tpriv->num_submitted);
			tpriv->num_submitted++;
			continue;
		}

		/* if the first URB failed, bail out */
		if (i == 0) {
			usbi_err(TRANSFER_CTX(transfer),
				"submiturb failed error %d errno=%d", r, errno);
			return errno == ENODEV ? LIBUSB_ERROR_NO_DEVICE
					       : LIBUSB_ERROR_IO;
		}

		/* if the error is transient, and there are still URBs queued,
		 + retry to submit later when URBs complete; when no more
		 * URBs are in flight then abort the transfer (since retries
		 * are handled from the URB completion handler)
		 */
		if ((errno == ENOMEM || errno == EAGAIN) &&
		    tpriv->num_retired < tpriv->num_submitted) {
			usbi_dbg("postponing submit at %d of %d, %d done, errno %d",
				 tpriv->num_submitted, tpriv->num_urbs,
				 tpriv->num_retired, errno);
			return 0;
		}
...
{
	//#define WEIRDLIMIT 12582912 //12Mb, this is the very limit as found by experiments. At least on 64Bit ubuntu and derivative
	#define WEIRDLIMIT 1048576 //1Mb, this is a sensible limit that looks attractive to both take advantage of gather-scatter and avoid problems with concurrent transactions
	 ...
	 */
	if ((dpriv->caps & USBFS_CAP_BULK_SCATTER_GATHER) && (transfer->length < WEIRDLIMIT))
	{
		/* Good! Just submit everything in one go */
		bulk_buffer_len = transfer->length ? transfer->length : 1;
		use_bulk_continuation = 0;
	} 
	else if (dpriv->caps & USBFS_CAP_BULK_SCATTER_GATHER)
	{
		// If transfer is above treshold don't count on a huge single urb 
		// and proceed with the split transfer 
		// also assume use of bulk continuation
		//bulk_buffer_len = MAX_BULK_BUFFER_LENGTH;
		bulk_buffer_len = WEIRDLIMIT;
	...
	r = submit_urbs(itransfer, transfer, tpriv, num_urbs);
...
static int op_cancel_transfer(struct usbi_transfer *itransfer)
{
	...
	return discard_urbs(itransfer, 0, tpriv->num_submitted);
}
	...
	struct usbfs_urb *urb)
{
	...
	if (urb_idx == tpriv->num_urbs - 1) {
		...
		/* if submit_urbs() fails, the number of URBs in flight for
		 * this transfer will decrease, but it will continue to work
		 * until no more URBs are queued; in this case submit_urbs()
		 * will discard the URBs and signal error
		 */
		submit_urbs(itransfer, transfer, tpriv, 1);
		if (SUBMIT_FAILED == tpriv->reap_action &&
		    LIBUSB_TRANSFER_COMPLETED == tpriv->reap_status)
			tpriv->reap_status = LIBUSB_TRANSFER_ERROR;
		if (tpriv->num_retired == tpriv->num_submitted)
			/* no more URBs in flight */
			goto completed;
		goto out_unlock;
	}
...
cancel_remaining:
	...
	if (tpriv->num_retired == tpriv->num_submitted) /* nothing to cancel */
		goto completed;

	/* cancel remaining urbs and wait for their completion before
	 * reporting results */
	discard_urbs(itransfer, urb_idx + 1, tpriv->num_submitted);

...
void qhy11_init()
{
	...
	qhy_core_getcampars()->buftimes   = 13000;
	qhy_core_getcampars()->buftimef   = 3000;
	...
	// Header values
	imgcam_get_camui()->pszx = 9.00;
	imgcam_get_camui()->pszy = 9.00;
...
int  qhy11_setregisters(qhy_exposure *expar) 
{
	int retval = 1;	
	int time, Vbin, Hbin, ShortExp, antiamp, PatchNumber;
	int top_skip = 0, bot_skip = 0;
	int top_skip_pix = 0, top_skip_null = 50;
	...
	switch ( bin ) 
	{
		case 1: i_width  = 4096; height = 2720; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		case 2: i_width  = 2048; height = 1360; width = i_width; Vbin = bin; Hbin = bin;
			break;
		case 3: i_width  = 1368; height =  906; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		case 4: i_width  = 1024; height =  680; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		default:
	...
	transfer_size = 512 * (int)(totalsize / 512);
	expar->tsize = transfer_size;
	
	PatchNumber = qhy_getPatch(totalsize, 512, 1);
	...
	REG[15]=expar->preview;	     	// LiveVideo no use for QHY7-8-9   16Bit set to 0
	REG[16]=0;
...
void qhy11_decode(unsigned char *databuffer)
{
	unsigned char *src1, *tgt;
	unsigned int   t, w;
	int left_skip = 0, right_skip = 0; 

	// It's for all bin modes
	if (width < i_width)
	{

...
void qhy6_init()
{
	...
	strcpy(imgcam_get_camui()->roistr, C_("camio","Full|288x288|144x144:0"));
	...
	// Header values
	imgcam_get_camui()->pszx = 6.50;
	imgcam_get_camui()->pszy = 6.25;
...
int  qhy6_setregisters(qhy_exposure *expar) 
{
	...
	switch ( bin ) 
	{
		case 1: i_width  = 800 ; height = 596; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		case 2: i_width  = 400 ; height = 298; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		default:
	...
	{
		top_skip = (int)((height - expar->height) / 4);
	...
	REG[47]=0;					// TopSkipPix no use for QHY9-11 16Bit set to 0 
	REG[48]=0;
...
void qhy6_decode(unsigned char *databuffer)
{
	...
	switch(bin) 
	{
		case 1:  //1X1 binning
			...
			if (width < i_width)
			{
				// Copy back from processed only the ROI data
			...
			{
				memcpy(databuffer, processed, allocsize);          
			}
			free(processed); 
			break;
		case 2:  //2X2 binning

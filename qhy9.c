...
#define PRODUCT_ID  0x8301
#define SHORTEXP    0
#define FWFILE      "qhy9.hex"
...
int  qhy9_setregisters(qhy_exposure *expar) 
{
	int retval = 1;	
	int time, Vbin, Hbin, ShortExp, antiamp, PatchNumber;
	int top_skip = 0, bot_skip = 0;
	int top_skip_pix = 0, top_skip_null = 30;
	...
	switch ( bin ) 
	{
		case 1: i_width  = 3584; height = 2574; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		case 2: i_width  = 1792; height = 1287; width = i_width; Vbin = bin; Hbin = bin;
			break;
		case 3: i_width  = 1196; height =  858; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		case 4: i_width  =  896; height =  644; width = i_width; Vbin = bin; Hbin = bin; 
			break;
		default:
...
void qhy9_decode(unsigned char *databuffer)
{

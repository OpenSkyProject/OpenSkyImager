...
//    int horzTotal=C->horzBefore+C->horzImage+C->horzAfter;
  ...
  if (C->amp == 1) 
  {
	ccd |= 0x8000; // START_SKIP_VDD = 0x8000
  }
  else
  {
	ccd &= ~0x8000;
  ...
  if (C->cameraID == ST237_CAMERA || C->cameraID == ST237A_CAMERA)
    {
...
OpenCCD (int CCD_num, CAMERA ** cptr)
{
  ...
  // Gather serial number & firmwareVersion  
  memcpy((*cptr)->serialNumber, eePtr.serialNumber, 10);
  (*cptr)->firmwareVersion = st.firmwareVersion;
  
  // Default amp mode (no control)
  (*cptr)->amp = -1;
